        ...
        <el-menu :default-active="$router.path"  router class="el-menu-vertical-demo">
          <el-submenu :index="index.toString()" v-show="isRole" v-for="(menu,index) in menus" :key="index">
            ...
            <el-menu-item-group v-if="cMenu.url" v-for="(cMenu,indexChild) in menu.childMenus" :key="indexChild">
              <el-menu-item :index="cMenu.url">{{cMenu.name}}</el-menu-item>
            </el-menu-item-group>
            <!--判断一级菜单下的子菜单没有url的-->
            <el-submenu v-if="!cMenu.url" :index="index.toString()+'-'+cMenu.order.toString()"
              ...
              <template slot="title">{{cMenu.name}}</template>
              <el-menu-item :index="sMenu.url"
                            v-for="(sMenu,indexSun) in cMenu.childMenus" :key="indexSun">{{sMenu.name}}
              </el-menu-item>
            </el-submenu>
          </el-submenu>
        </el-menu>
